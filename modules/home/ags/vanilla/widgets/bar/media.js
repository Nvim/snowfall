...
export function MediaWidget() {
  return Widget.Window({
    ...
    child: Widget.Box({
      ...
      vertical: true,
      spacing: 8,
      // css: "min-height: 2px; min-width: 2px; margin: 10px;", // small hack to make it visible
...
const getLabel = (player) => {
  ...
  Widget.Label({
    label: `${track_artists.join(', ')} - ${track_title}`,
  })
}
...
export function MediaLabel() {
  return Widget.Button({
    ...
    onClicked: () => {
      App.toggleWindow("media-widget")
    },
    child: Widget.Label("Music")
  })
}
