...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    # Hide GTK title bars that don't respect ssd: (https://codeberg.org/river/wiki#global-workaround-using-gtk-css)
    # extraCss is in stylix config
    gtk.gtk4 = {
      enable = true;
      extraConfig = {
    ...
    gtk.gtk3.extraConfig = config.gtk.gtk4.extraConfig;
    gtk.gtk3.extraCss = config.gtk.gtk4.extraCss;
      ...
      extraConfig =
        ...
        lib.strings.concatStrings [
          ''
            ...
            riverctl rule-add -title 'ghostty' tags $((1 << 0)) 
            ...
            riverctl map normal Super B spawn 'firefox --ozone-platform=wayland'
